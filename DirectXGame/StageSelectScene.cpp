...
void StageSelectScene::Initialize() {
	...
	// ステージポイントの配置 (3x3のグリッド)
	float intervalX = 5.0f;
	float intervalY = 4.0f;
	float offsetX = (float)(3 - 1) * intervalX / 2.0f;
	float offsetY = (float)(3 - 1) * intervalY / 2.0f;

	for (int i = 0; i < kMaxStage; i++) {
		stageTransforms_[i].Initialize();
		// 行と列の計算
		int row = i / 3; // 0:上段, 1:中段, 2:下段
		int col = i % 3; // 0~2:左から右
...
void StageSelectScene::Update() {
	...
	case Phase::kMain:
		...
		if (Input::GetInstance()->TriggerKey(DIK_LEFT)) {
			if (currentStage_ % 3 > 0) {
				currentStage_--;
			}
		}
		if (Input::GetInstance()->TriggerKey(DIK_RIGHT)) {
			if (currentStage_ % 3 < 2) {
				currentStage_++;
			}
		}
		// 十字キー上下で行移動
